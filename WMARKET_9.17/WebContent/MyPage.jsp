...
<%
	// tab을 초기화 시키고
	// 'tab'이라는 데이터가 넘어온 것이 존재한다면 캐스팅을 하여 변수에 담는다
	String tab = null;
...
window.onload = function(){
	var tab = "<%=tab%>";
	if (tab == "acc") {
		const tabBtn = document.querySelector('#account-tab');
		tabBtn.className = "nav-link active";
		tabBtn.ariaSelected = "true";
		
		const tabBtn2 = document.querySelector('#product-tab');
		tabBtn2.className = "nav-link";
		tabBtn2.ariaSelected = "false";
		
		const tabCon = document.querySelector('#myAccount');
		tabCon.className = "container tab-pane fade show active";
		
		const tabCon2 = document.querySelector('#myProduct');
		tabCon2.className = "container tab-pane fade";
	}
}
</script>
</head>
...
<ul class="nav nav-tabs" id="myTab" role="tablist" style="padding-left: 12%;">
  <li class="nav-item" role="presentation">
    <button class="nav-link active" id="product-tab" data-bs-toggle="tab" data-bs-target="#myProduct" type="button" role="tab" aria-controls="myProduct" aria-selected="true">내 상품</button>
  </li>
  <li class="nav-item" role="presentation">
    <button class="nav-link" id="bid-tab" data-bs-toggle="tab" data-bs-target="#myBid" type="button" role="tab" aria-controls="myBid" aria-selected="false">내 입찰</button>
  </li>
  <li class="nav-item" role="presentation">
    <button class="nav-link" id="account-tab" data-bs-toggle="tab" data-bs-target="#myAccount" type="button" role="tab" aria-controls="myAccount" aria-selected="false">예치금</button>
  </li>
</ul>
...
<% //마감된 상품들
	...
	for(int i = 0; i < list2.size(); i++){
		...
		bidMaxPrice = bidDAO.bidMaxPrice(list2.get(i).getpNumber());
		
		int random = (int)(Math.random()*1000);	//modalId에 붙여줄 random 값
		modalID = "modal_"+pNumber+random;
...
<% //유효한 상품들
	...
	for(int i = 0; i < list3.size(); i++){
		...
		path = list3.get(i).getImage(); //이미지 경로
		File dir = new File(path);
...
<% //마감된 상품들
	...
	for(int i = 0; i < list4.size(); i++){
		...
		int random = (int)(Math.random()*1000);	//modalId에 붙여줄 random 값
		modalID = "modal_"+pNumber+random;
		modalID2 = "report_"+pNumber+random;
